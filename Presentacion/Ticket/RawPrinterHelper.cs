...
//#region https://code.google.com/p/itraacv2-1/source/browse/trunk/App/Helpers/RawPrinterHelper.cs?r=14
//﻿using System;
...
//      columnRuler2 += (((i == 1 ? 0 : -1) + i).ToString(CultureInfo.InvariantCulture) + "         ").PadLeft(10);
//    }
//    columnRuler1 += "0123456789".PadLeft(PageWidth - columnRuler1.Length);
//    columnRuler2 +=
//      ((i - 1).ToString(CultureInfo.InvariantCulture) + "         ").PadLeft(PageWidth - columnRuler2.Length);
...
//  public void PrintStringAtPos(string str, int pagenum, int col, int row)
//  {
//      int maxlength = 100; int maxrows = 1;
...
//          str = str.PadLeft(-result.Value.Length);
//          //negative PadLeft "slices" x chars off the PadLeft and returns the remainder
//        }
//        //if we bailed out because we couldn't wordwrap anything in, just include the chopped text so at least it's visible
//        if (lines.Count < Math.Abs(maxrows) && str.Length > 0) lines.Add(str.PadLeft(maxlength));
//      }
...
//      PrintBytesAtPos(CP437GetBytes(str.PadLeft(maxlength)), pagenum, col, row);
//  }
