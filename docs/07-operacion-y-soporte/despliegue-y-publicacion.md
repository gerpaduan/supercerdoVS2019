...
- Ubicacion: `AFIP/<CUIT>/` dentro del sitio. Desde 2026-09-25 la app guarda un certificado por entorno en `AFIP/<CUIT>/prod/` y `AFIP/<CUIT>/homo/` (nombre fijo `certificado.pfx`); el pfx historico de la raiz (`certif-prod.pfx`, tickets, `LoginTemplate.xml`) sigue funcionando como respaldo de solo lectura para produccion y **no se toca ni se mueve**. Migracion `DatosPostgres/DB-Migrations/20260925-Alter_certificado_arca_clave_entorno.sql` (clave por entorno) como `carnisys_admin` **antes** del codigo (aplicada en la base local el 2026-09-25; PENDIENTE en la VPS luden). SM y San Lorenzo (SQL Server) no aplican. Es estado vivo: **nunca se toca en un deploy** (IIS: ya estaba en la lista de "no tocar"; Docker/luden: volumen `carnisys-afip` montado en `/app/AFIP`, creado con dueno `app`; agregado en `deploy/luden/docker-compose.yml` y `Dockerfile`).
...
## Redeploy WebCore en Servidor SM -- 2026-09-24 (commit `eb1af9e5` + ajuste de menu sin commitear; tag `pre-deploy-sm-20260924`)
- **Que se publico**: WebCore completo (Pagos con eliminacion logica y auditoria, POS latido al cerrar pestana, modo prueba de AFIP, reglas de rutas/tickets de AFIP). Certificados de plataforma/Postgres no aplican a este servidor (SQL Server, una sola empresa): las pantallas de certificado no se muestran.
- **Procedimiento seguido (el documentado arriba)**: `dotnet publish -c Release -r win-x64 --self-contained true` -> se **sacaron `App.config` y `WebCore.dll.config` del publish** (son los locales, de Postgres) -> zip subido por SFTP a `C:\WebCore_upload` (hash SHA256 verificado) -> watchdog deshabilitado y app detenida -> backup `C:\WebCore_backup_20260924` (175 MB) -> `Expand-Archive -Force` sobre `C:\WebCore` (config del servidor intacta, verificado por hash) -> migracion -> arranque -> watchdog habilitado. Ventana de corte ~3 minutos.
- **Migracion SQL Server aplicada**: `Datos/DB-Procedures/20260924-Alter_Pagos_Eliminado_Create_AuditoriaPagos.sql` sobre `SuperCerdo` (columnas `eliminado*` en `Pagos` + tabla `AuditoriaPagos`; 12.907 pagos quedaron con `eliminado = 0`).
- **Certificado de AFIP para facturar**: `C:\WebCore\AFIP\20306210786\` con `certif-prod.pfx` (copiado en el servidor desde `C:\inetpub\wwwroot\CarniSysWeb\AFIP\20306210786\`, mismo SHA256 que el usado en desarrollo; vence 2028-06-10; sin clave) y `LoginTemplate.xml`. WebCore corre con `WorkingDirectory=C:\WebCore`, por eso la carpeta va ahi (la clasica tiene la suya). Ademas se copio un `TicketAcceso.txt` vigente hasta 2026-09-25 07:58: ARCA no entrega otro ticket mientras hay uno valido, y ese ticket lo habia pedido la PC de desarrollo (prueba de conexion). Pasada esa hora el servidor pide los suyos solo.
- **Validado**: `http://127.0.0.1:5250/Login` 200 desde el servidor; `https://192.168.0.151/Login` 200 (titulo "Ingresar a CARNISYS"); `/SuperCerdoWeb/` 200; `/CarniSysWeb/Login/Index` 301; tareas `WebCoreApp` corriendo y `WebCoreAppWatchdog` Ready; el servidor alcanza `wsaa.afip.gov.ar`, `servicios1.afip.gov.ar` y `aws.afip.gov.ar` por 443.
- **PENDIENTE (a cargo del usuario)**: login real y una factura de prueba de punta a punta (no se emitio ningun comprobante ni se hizo login con un usuario real en esta sesion).
- **Rollback**: detener el watchdog y `WebCoreApp`, `robocopy C:\WebCore_backup_20260924 C:\WebCore /MIR` **excluyendo** `AFIP\`, arrancar y habilitar el watchdog. Las columnas nuevas de `Pagos` son aditivas y el codigo viejo las ignora: no hace falta revertirlas.

## Redeploy WebCore en San Lorenzo -- 2026-09-25 (mismo paquete que Servidor SM: commit `eb1af9e5` + ajuste de menu, tag `pre-deploy-sm-20260924`)
- **Antes**: el SSH de San Lorenzo cortaba la conexion apenas empezaba la autenticacion, solo para `supercerdoSL` (un usuario inexistente respondia normal); se resolvio con `Restart-Service sshd` hecho por el usuario en el servidor. Motivo de fondo no determinado (el log de OpenSSH solo mostraba intentos de fuerza bruta de internet contra usuarios inexistentes: el servidor esta expuesto, ver `BASELINE_EXCEPTIONS` en `~/hosts/sanlorenzo.env`).
- **Pasos**: zip subido por SFTP a `C:\inetpub\wwwroot\web\_deploy` (SHA256 verificado) -> backup `CarniSysWeb-backup-20260925` (165 MB, `Copy-Item`) -> zip descomprimido en staging (sin `WebCore.dll.config` ni `App.config`) -> **migracion `Datos/DB-Procedures/20260924-...sql` aplicada sobre `supercerdo`** (668 pagos, todos `eliminado = 0`) -> `Stop-WebAppPool CarniSysWebCore` -> `robocopy <staging> CarniSysWeb /E /XF WebCore.dll.config App.config` (sin `/MIR`; configuracion intacta, verificada por hash) -> carpeta `CarniSysWeb\AFIP\20306210786\` con `certif-prod.pfx` y `LoginTemplate.xml` copiados desde `CarniSysWeb-backup-20260916\AFIP\20306210786\` (mismo SHA256 que SM y desarrollo; vence 2028-06-10; sin clave) -> `Start-WebAppPool`. Solo se toco el pool `CarniSysWebCore`.
- **Validado**: `https://200.107.108.44/CarniSysWeb/Login/Index` 200 ("Ingresar a CARNISYS"), CSS 200, `/SuperCerdoWeb/` 200, pools `CarniSysWebCore`/`web`/`SuperCerdo` Started, sin errores en el log de Application; el servidor alcanza `wsaa`, `servicios1` y `aws.afip.gov.ar` por 443; `IIS_IUSRS` tiene control total sobre `AFIP\` (hereda).
- **A tener en cuenta (facturar desde dos servidores)**: SM y San Lorenzo usan el MISMO certificado (CUIT 20306210786). ARCA no entrega un ticket nuevo mientras hay uno vigente (12 h) para el mismo certificado y servicio, aunque lo pida otra maquina: si SM y San Lorenzo facturan con menos de 12 h de diferencia, el segundo puede recibir "ya posee un TA valido" hasta que venza el ticket del primero. Ya existia con la clasica; no hay ticket copiado en San Lorenzo (el de desarrollo vencio).
- **PENDIENTE (a cargo del usuario)**: login real y una factura de punta a punta. No se emitio ningun comprobante.
- **Rollback**: `Stop-WebAppPool CarniSysWebCore`, `robocopy CarniSysWeb-backup-20260925 CarniSysWeb /E` (sin pisar `AFIP\`), `Start-WebAppPool CarniSysWebCore`. Las columnas nuevas de `Pagos` no hace falta revertirlas.

## Redeploy WebCore en VPS luden -- 2026-09-25 (imagen `carnisys-web:20260925-1149`; anterior `20260923-2105`; tag `pre-deploy-luden-20260925`)
- **Base: solo migraciones, NO se reemplazo por la local.** La VPS tenia datos propios posteriores a la copia del 23/09 (8 ventas del 24/09 y 5 facturas electronicas; 577.100 ventas vs 577.092 locales), asi que reemplazarla los habria perdido. Decision del usuario: conservar la base de la VPS.
- **Backup previo**: `/srv/carnisys/backups/carnisys-pre-20260925-1148.dump` (43 MB, chmod 600).
- **Migraciones aplicadas como `carnisys_admin`** (`psql -v ON_ERROR_STOP=1 --single-transaction`), en este orden: `20260924` (pagos eliminado + auditoriapagos), `20260924b` (esprueba + certificado_arca_clave), `20260924c` (notificaciones.tipo varchar(60)), `20260924d` (plataforma_certificado_arca). Base: 58 -> 61 tablas. Ultima aplicada en la VPS: `20260924d`.
- **Compose**: `/srv/carnisys/docker-compose.yml` reemplazado por `deploy/luden/docker-compose.yml` (suma el volumen `carnisys-afip:/app/AFIP`; copia previa `docker-compose.yml.bak-20260925`). `WebCore.dll.config` de la VPS sin cambios (las claves `Afip:*` tienen default en codigo).
- **Verificado**: `web` y `db` healthy, `https://carnisys.com/Login` 200, `/app/AFIP` escribible por `app`, 0 excepciones en el log, los otros contenedores del host intactos. **NO probado**: login real, generar CSR/subir certificado en la VPS (carpeta `AFIP` vacia: no hay certificado cargado, la facturacion desde luden requiere cargarlo por Configuracion > Certificado ARCA).
- **Rollback de codigo**: re-taggear `carnisys-web:20260923-2105` como `latest` y `docker compose up -d web`. Las migraciones son aditivas: el codigo viejo las ignora.

**Actualizacion 2026-09-25 (13:40): certificados por entorno (commit `d379cd1e`, imagen `carnisys-web:20260925-1341`)**: backup `/srv/carnisys/backups/carnisys-pre-20260925-1340.dump` -> migracion `20260925-Alter_certificado_arca_clave_entorno.sql` como `carnisys_admin` (PK de `certificado_arca_clave` pasa a `(idempresa, entorno)`; 0 filas) -> publish desde un **worktree limpio del commit** (el arbol de trabajo tenia cambios sin commitear de otra sesion que no se publicaron) -> build -> `up -d web`. Verificado: `web` y `db` healthy, `https://carnisys.com/Login` 200, 0 excepciones, `/app/AFIP` escribible, otros contenedores intactos, 577.100 ventas (sin cambios). La carpeta `AFIP` de la VPS sigue vacia: el certificado se carga desde Configuracion > Certificado ARCA (produccion o pruebas, cada uno en `AFIP/<CUIT>/prod|homo`). NO probado en la VPS: login real ni carga de certificado. Ultima migracion aplicada en la VPS: `20260925`. Rollback de codigo: re-taggear `carnisys-web:20260925-1149` como `latest` y `up -d web` (la migracion es aditiva salvo la PK: con codigo anterior guardar una clave de pfx fallaria; volver la PK a `(idempresa)` solo si hace falta y no hay filas HOMO).
