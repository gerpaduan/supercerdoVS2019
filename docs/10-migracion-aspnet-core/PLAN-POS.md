...
**No implementado todavía** (batches 5-7 del plan): expendios asociados (`BuscarExpendiosPOS`/`ObtenerExpendioPOS`), código de barras real con formato configurado (falta la tabla `FormatosCodigoBarras` en la base local), enganche con AFIP desde una venta real de POS (no la venta manual ya probada), `PuntosExpendioController.POS`/`FinalizarPOS`, y toda la vista de POS en sí (`Ventas/POS.cshtml`, 6226 líneas -- nada de esto tiene UI en WebCore todavía, solo los endpoints de arriba, verificados por HTTP directo).
