...
- **`BuscarProducto`** -- primera versión (2026-09-03) usaba la lógica estable de `PuntosExpendioController.BuscarProductoPOS` porque `Negocio.BarcodeInterpreter` (de la sesión en paralelo) todavía no compilaba. **Actualizado (2026-09-04)**: esa sesión commiteó su trabajo (`e92eae9d "Motor configurable de codigos de barra internos de balanza (EAN-13 prefijo 20-29)"`), verificado que compila limpio en los 2 TFM de `Negocio.csproj` (`dotnet build` net10.0 y `MSBuild.exe` net472), y se actualizó `BuscarProducto` para usar la clase real `Negocio.BarcodeInterpreter` (mismo código que el original, sin adaptar). Probado con datos reales: `BuscarProducto?codigo=1` (código corto, camino "de siempre") devolvió CARRE/id=33/$12000 vía el motor nuevo; un EAN-13 de 13 dígitos con prefijo 20-29 sin checksum válido degradó correctamente a "Código inexistente" sin romper (la tabla `FormatosCodigoBarras` todavía no existe en la base local de dev -- no se llegó a ejercitar el camino de formato configurado real, pero el manejo de excepción de `Interpretar()` ya garantiza que no rompe el endpoint si esa tabla falta).
- **`AgregarProducto`**: confirmado código muerto (sección 2), no se porta.

**Pago mixto verificado con datos reales** (2026-09-04): `FinalizarVenta` con `formaPago=Debito`, `esPagoMixto=true`, `efectivo=5` → venta real `idVenta=1740`, confirmado en `Ventas.pagoMixtoEfectivo=5.0`. Ya estaba soportado en el código portado (batch 3), solo faltaba probarlo -- sin cambios de código.

**Expendios asociados, portado y verificado con datos reales** (2026-09-04): `BuscarExpendiosPOS`/`ObtenerExpendioPOS` portados sin cambios de lógica (`Session["Usuario"]` -> `_usuarioActual`, `JsonRequestBehavior.AllowGet` no aplica en Core). Probado contra la base local real (sucursal 2, 21 expendios existentes): `BuscarExpendiosPOS?estado=Todos` devolvió los 21 con `asignado`/`cargadoEnVentaActual` correctos; `estado=Pendientes` (default) filtró correctamente los ya asignados a una venta; `ObtenerExpendioPOS?idExpendio=82` devolvió el expendio real (NICOLAS, 2 líneas: CARRE 1kg balanza, CABEZA 3kg) y `idExpendio=999999` devolvió `{ok:false,"msg":"El expendio no existe."}` como el original.

**Batch 7 (`PuntosExpendioController.FinalizarPOS`), portado y verificado con datos reales** (2026-09-04): mismo patrón que `FinalizarVenta` -- sin cambios de lógica respecto al original más allá de `Session["Usuario"]` -> `_usuarioActual` y omitir `ResolverOperadorPOS` (bajo el stub Admin=true, `EsUsuarioProduccion` siempre es `false`, así que ese método devolvería el mismo usuario sin cambios -- código muerto bajo el stub, mismo criterio de siempre). Se agregaron `WebCore/Models/PuntoExpendioVm.cs` (`PuntoExpendioEditVm`/`PuntoExpendioLineaVm`, antes excluidos a propósito) y `WebCore/Models/DTO/FinalizarPuntoExpendioRequest.cs`. Probado: rechazo de sector vacío y de sector inexistente (mismos mensajes que el original), y un expendio real exitoso (`idExpendio=95`, sector "Carniceria", 1 línea CARRE 1kg a $12000, `idCorte=33` resuelto vía `Ventas/BuscarProducto?codigo=1`) -- verificado leyéndolo de vuelta con `ObtenerExpendioPOS?idExpendio=95` (batch 5) y confirmando que su PDF (`ImprimirPdf/95`) genera un documento válido de 1 página. `PuntosExpendioController.POS` (la vista en sí, 1980 líneas) sigue sin portar -- no tiene sentido sin la UI real de POS.

**Batch 6 (enganche AFIP desde una venta real de POS), verificado -- sin código nuevo** (2026-09-04, autorizado explícitamente por el usuario, "correlo con un importe menor a 50 pesos"): confirmado que `GenerarFactura`/`MapDtoToFactura` ya son agnósticos del origen de la venta -- `factura.Venta = _oVentaN.getVentaById(dto.IdVenta)` resuelve la venta real sin importar si vino del flujo manual (`CrearVentaManualParaFactura`) o de un `FinalizarVenta`/`FinalizarPOS` real, y no usa `dto.Detalle` (las líneas salen de `factura.Venta.LineasVenta`, cargadas de la base). Se usó `PreviewFacturaDto?idVenta=1740` (venta real del batch de pago mixto, $15, "Debito | Efectivo", persona Consumidor Final sin CUIT -- mismo perfil de persona que ya funcionó en el mini-spike de AFIP original) para armar el POST a `GenerarFactura` (form-urlencoded, sin `Detalle`). **Resultado real de producción AFIP**: Factura B nro `00056326`, CAE `86361436415713`, `facturaId=113` -- verificado persistido leyendo `Ventas/DetalleFactura?id=113` (CAE y número aparecen en el HTML renderizado). Con esto el núcleo POS transaccional completo (batches 1-7) está portado y verificado con datos/AFIP reales; solo quedan balanza física y código de barras con formato configurado real (bloqueados por infraestructura/datos que no existen en el entorno local de dev) y la UI de POS en sí.

**No implementado todavía**: código de barras real con formato configurado (falta la tabla `FormatosCodigoBarras` en la base local de dev), balanza física, y toda la vista de POS en sí (`Ventas/POS.cshtml`, 6226 líneas; `PuntosExpendio/POS.cshtml`, 1980 líneas -- nada de esto tiene UI en WebCore todavía, solo los endpoints, verificados por HTTP directo).
