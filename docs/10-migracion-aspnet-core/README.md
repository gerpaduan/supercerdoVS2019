...
**Pendiente para una próxima sesión**: portar `GenerarPdfComprobanteBytes`/`GenerarPdfDetalleVentaBytes` (Ventas, usa `Utilidades.GenerarDocs.GenerarFacturaPDF`, no leído todavía) y los PDF de `FinanzasController`; conectar `ObtenerDatosEmailComprobante`/`EnviarComprobanteEmail` (Ventas) y `EnviarCuentaCorrienteEmail`/`EnviarComprobantePagoEmail` (Finanzas); reemplazar el flujo de email `prompt()`-based de PuntosExpendio por un modal real cuando se porte el resto.
