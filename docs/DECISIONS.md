...
## 2026-09-14 - Cutover de producción: `carnisys.com` pasa de la clásica (IIS) a WebCore standalone sobre Postgres

**Pedido**: instalar Postgres en la VM de producción "CarniSys" y publicar WebCore para que corra en `carnisys.com`, con la clásica en `/clasica`. Investigando por SSH aparecieron 2 hechos reales que cambiaron el pedido: (1) Postgres ya estaba instalado y en uso productivo en esa VM desde el 2026-08-29 (`docs/07-operacion-y-soporte/despliegue-y-publicacion.md`); (2) `carnisys.com` estaba **caído en producción** en el momento de la investigación -- `/Login/Index`/`/Home/Index` devolvían `404`, sólo `/` respondía. Causa real: el sitio IIS "CarniSys" (el build correcto, del 29/8) estaba `Stopped` por una razón anterior y no relacionada a este trabajo; el tráfico caía por el binding sin hostname de IIS a un sitio "web" con una copia de julio desactualizada, donde las rutas no matcheaban. La doc de despliegue describía un estado que ya no era el real (corregida en el mismo commit, CLAUDE.md §8.3).

**Redefinición del pedido por el usuario, tras ver estos hallazgos**: WebCore corre standalone, independiente de IIS, ya apuntando a Postgres, publicado de inmediato. Se descartó `/clasica`: la web clásica se da de baja de esta VM (sitios IIS detenidos, no borrados de disco).

**Arquitectura elegida**:
- **Publish self-contained `win-x64`** (`dotnet publish -c Release -r win-x64 --self-contained true`): la VM no necesita instalar ningún runtime .NET ni el Hosting Bundle de IIS -- resuelve "independiente de IIS" de punta a punta. Alternativa descartada: framework-dependent + IIS con ASP.NET Core Module (ANCM) -- se descartó por agregar un paso de instalación evitable.
- **Proceso administrado por Tarea Programada + Watchdog**, replicando 1:1 el patrón ya probado en esta VM para Caddy (`C:\caddy\{caddy.exe, watchdog.ps1, register-watchdog.ps1}`, tareas `Caddy`/`CaddyWatchdog` desde 2026-06-15) -- cero dependencias nuevas (se descartó instalar NSSM/WinSW). Tareas `WebCoreApp` (lanza `C:\WebCore\WebCore.exe --urls http://127.0.0.1:5250`) y `WebCoreAppWatchdog` (cada 2 min, SYSTEM, backstop vía `Get-NetTCPConnection`).
- **Kestrel bindea solo `127.0.0.1`** (mismo criterio de seguridad que ya aplica Postgres en esta VM -- nunca expuesto directo a la red).
- **Base de datos y SMTP reusados de la clásica**: `C:\WebCore\App.config` (gitignorado, generado una vez a mano en la VM) usa la MISMA `ConexionPostgresPiloto` (host, base, rol `carnisys_user`) y el mismo SMTP real que ya tenía la clásica en `Config\connectionStrings.config`/`Config\appSettings.secrets.config` de esa VM -- extraídos y sustituidos en un único script remoto server-side, sin que los valores reales pasaran por el chat/output en ningún momento.
- **Caddy**: `Caddyfile` reemplazado (backup `Caddyfile.bak-20260914-webcore-cutover`) -- un solo `reverse_proxy localhost:5250` para `carnisys.com`, sin el compat-redirect viejo de `/CarniSysWeb/*` (ya no aplica, la clásica se retiró).

**Bug real encontrado y corregido durante el cutover**: registrar `WebCoreAppWatchdog` copiando literalmente el patrón de Caddy (trigger `-AtStartup` con un `Repetition` pegado desde un trigger `-Once` descartado) dejaba `NextRunTime` vacío -- la tarea nunca arranca su ciclo de 2 minutos porque el trigger base (`AtStartup`) nunca disparó (la VM no reinició desde que se registró la tarea), y sin ese primer disparo la repetición adosada no arranca sola. Confirmado en vivo: se mató el proceso a mano y `carnisys.com` quedó en `502` sin recuperarse. **Fix**: 2 triggers separados en la misma tarea -- uno `-Once -At (+30s) -RepetitionInterval 2min -RepetitionDuration 3650días` (arranca el ciclo de inmediato) más uno `-AtStartup` (para reiniciar el ciclo también tras un reboot real). Re-verificado en vivo: proceso matado a mano -> `watchdog.log` registró `CAIDO -> reinicio tarea` y `RECUPERADO` 9 segundos después, sitio de nuevo en `200` dentro del ciclo esperado. **Hallazgo colateral, no corregido**: `CaddyWatchdog` (la referencia de este patrón, en producción desde 2026-06-15) probablemente tiene el mismo problema estructural -- funciona hoy porque la VM ya reinició al menos una vez desde su registro, no porque el patrón sea correcto. Fuera de alcance de este cutover; si algún día se re-registra esa tarea con `-Force` sin corregir el trigger, quedaría con el mismo bug latente.

**Otros ajustes**: se ocultó el único link de WebCore que apuntaba a la clásica (Mercado Pago, `_Layout.cshtml`, vía `WebClasicoBaseUrl`) -- es el único módulo todavía no portado; con la clásica retirada de esta VM ese link quedaba roto (`href=""`). Se eliminó también el código muerto asociado (`webClasico`/`legacy()`, sin más usos en el archivo).

**Verificado en vivo, de punta a punta**: `curl https://carnisys.com/` y `/Login` -> `200`, header `Server: Kestrel` (confirma que sirve WebCore, no la clásica); título de login `Ingresar a CARNISYS`. Sitios IIS "CarniSys"/"web" confirmados `Stopped` sin afectar el tráfico. Suite E2E local de WebCore ya en 190/190 contra Postgres local -- no se re-certificó funcionalidad en este cutover, sólo que el despliegue llegara sano (los checks de arriba).

**Deuda/pendiente**: no se verificó el módulo Mercado Pago en producción (queda oculto hasta que se porte a WebCore). No se corrigió el bug latente de `CaddyWatchdog` (documentado arriba, fuera de alcance). `docs/07-operacion-y-soporte/despliegue-y-publicacion.md` actualizado con el procedimiento completo y la sección vieja de esta VM marcada como historia obsoleta.
