...
**`UsuarioPasskeys` (2026-09-21, login por huella)**: tabla nueva `usuariopasskeys` con RLS por `idempresa` (mismo patron que `usuariopasswordresettokens`, sin excepcion `OR idempresa=0`). El login por huella busca la credencial por `credentialid` sin conocer el tenant, asi que `DatosPostgres/UsuarioPasskeyPg.cs` asume `SET LOCAL ROLE carnisys_usuarios_bypass` (el rol recibio `SELECT, UPDATE` sobre la tabla en la migracion `20260921-Create_usuariopasskeys.sql`); el resto de los metodos (listar/alta/baja de las huellas del propio usuario) van por el tenant normal. Ver `docs/DECISIONS.md` "Login por huella (passkeys WebAuthn)".
