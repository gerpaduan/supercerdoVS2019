...
{
    // Port de Web/Helpers/SystemAdministrationRepositoryPg.cs (2026-09-05, ver docs/DECISIONS.md
    // "Postgres es la base oficial y unica" -- gap encontrado en la auditoria de paridad con
    // Playwright: SystemAdministrationController nunca seguia el switch hibrido, siempre leia SQL
    // Server). Adaptador Postgres del modulo de administracion de plataforma: implementa
    // ISystemAdministrationRepository traduciendo WebCore.Models.* <-> Entidades.* en memoria (sin
    // SQL propio, todo el acceso a datos vive en DatosPostgres.SystemAdministrationPg, la misma
    ...
    public class SystemAdministrationRepositoryPg : ISystemAdministrationRepository
    {
        ...
        public SystemAdministrationRepositoryPg(string connectionString)
        {
            _repo = new DatosPostgres.SystemAdministrationPg(connectionString);
        }

        // "telefono"/"activa" de Sucursal nunca existieron en Postgres (mismo hallazgo que el
        // clasico, ver Web/Helpers/SystemAdministrationRepositoryPg.cs), asi que este modulo
        // asume ambas columnas siempre ausentes: mismo fallback que SystemAdministrationRepository
