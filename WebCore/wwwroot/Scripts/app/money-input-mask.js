...
//
// Extension 2026-09-07 respecto al original del clasico (ver docs/DECISIONS.md, port a
// Productos/Index.cshtml): cada tecla intercepta con preventDefault, asi que el navegador NUNCA
// dispara su propio evento 'input' nativo -- el clasico no lo necesitaba (Finanzas/Pagos no tiene
// nada mas escuchando ese campo), pero WebCore si (actualizarVariacionPrecio recalcula "+10%"/
// "Sin variacion" en vivo). El keydown handler ahora dispara $(input).trigger('input') el mismo
// DESPUES de cada render(), para que listeners externos se enteren del cambio -- sin loop: el
// listener de 'input' que ya tenia attach() (para resincronizar cuando algo EXTERNO escribe el
// valor) solo llama a render() de nuevo con el estado ya correcto, nunca vuelve a disparar el
// evento el mismo.
//
...
(function (window, $) {
    ...
    function attach(selector) {
        ...
        // Renderiza y avisa a listeners externos (ej. actualizarVariacionPrecio en
        // Productos/Index.cshtml, enganchado con addEventListener nativo) que el valor cambio --
        // ver comentario de "Extension 2026-09-07" mas arriba. dispatchEvent nativo, NO
        // $(input).trigger('input') -- jQuery.trigger no siempre llega a los listeners agregados
        // via addEventListener nativo (confirmado con Playwright: el jQuery synthetic event no
        // disparaba el listener de Productos/Index.cshtml). No causa loop: el listener de 'input'
        // de abajo solo re-renderiza el mismo estado ya correcto, nunca vuelve a llamar a esta
        // funcion.
        function renderYNotificar() {
            render(input, estado);
