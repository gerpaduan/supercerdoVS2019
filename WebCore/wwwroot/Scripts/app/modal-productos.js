// Port literal de Web/Scripts/app/modal-productos.js (buscador avanzado de producto, F10),
// 2026-09-06 (retomado -- ver docs/DECISIONS.md). Modulo generico y reutilizable (lo usa el POS
// y tambien Elaborados/EditarFormula.cshtml) -- window.abrirBuscarProductoModal(opts) es la API
// publica que ya consumia pos-product.js (openSearchModal), portado sin cambios en su momento
// pero inerte hasta ahora por falta de este archivo. Sin cambios de logica.
(function () {
    ...
    function getColspanMensajes(state) {
        // +1 por la columna "#" (siempre visible, a diferencia de precio que se puede ocultar).
        return (state.mostrarPrecio ? 3 : 2) + 1;
    }
    ...
    function initModalIfNeeded($modal) {
        ...
        // Enter -> seleccionar activa (o primera); Alt+1..9 -> seleccionar la fila N directamente
        $input.on('keydown.buscarProducto', function (e) {
            ...
            if (digit != null) {
                ...
                var $rowDigit = $tbody.find('tr').eq(digit - 1);
                if ($rowDigit.length && !esFilaMensaje($rowDigit)) {
                    seleccionarProductoDesdeRow($modal, $rowDigit);
                }
            }
        });
