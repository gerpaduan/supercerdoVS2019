// Port literal de Web/Scripts/app/pago-cheques.js (2026-09-05, ver docs/DECISIONS.md -- pago con
// cheque en AddOrEditPago). 100% client-side, sin cambios de logica: los 3 endpoints que consume
// (buscarChequePorNro/getCheques/guardarCheque) ya existian en WebCore/Controllers/FinanzasController.cs
// con el mismo contrato JSON que este script espera (confirmado leyendo el codigo antes de portar).
//
// AddOrEditPago.cshtml no puede usar @section Scripts (tambien se sirve sin layout via
// PartialView), asi que este <script src> puede ejecutar antes de que jquery.min.js cargue --
// mismo guard de espera-activa que ya usa modal-cheque.js (window.jQuery undefined -> reintenta).
(function (window) {
    function inicializarPagoCheques() {
        var $ = window.jQuery;
        if (!$) {
            window.setTimeout(inicializarPagoCheques, 60);
            return;
        }

        if (window.PagoCheques && window.PagoCheques.__inicializado) {
            return;
        }

    window.PagoCheques = (function () {
        ...
        // Hallazgo real (2026-09-05, ver docs/DECISIONS.md): a diferencia de MVC5 clasico (Json()
        // preservaba el PascalCase de los objetos anonimos del C#), el Json() de ASP.NET Core usa
        // camelCase por defecto (id/nroCheque/banco/... en vez de Id/NroCheque/Banco/...) -- el
        // resto de este archivo (port literal del original) espera PascalCase. Se normaliza una
        // sola vez en el borde donde entran datos del servidor, en vez de tocar cada lectura de
        // propiedad de la logica ya portada. El seed que arma el propio servidor (PagoChequeSeed,
        // escrito directo como literal JS en AddOrEditPago.cshtml, no pasa por Json()) ya viene en
        // PascalCase -- por eso el chequeo `c.Id !== undefined` hace que esta funcion sea inerte
        // para ese caso.
        function normalizarChequeServer(c) {
            if (!c || c.Id !== undefined) return c;
            ...
            return {
                ...
                Origen: c.origen,
                Titular: c.titular,
                RecibidoDeNombre: c.recibidoDeNombre,
                EntregadoANombre: c.entregadoANombre,
        ...
        function agregarFilaCheque(cheque, opciones) {
            ...
            cheque = normalizarChequeServer(cheque);
            if (!cheque || !cheque.Id) {
                return false;
            }
        ...
        function renderizarTablaBusqueda(lista) {
            ...
            lista = lista.map(normalizarChequeServer);
