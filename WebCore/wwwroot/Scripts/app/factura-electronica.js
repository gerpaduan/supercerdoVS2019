// Port de Web/Scripts/app/factura-electronica.js (modal de Factura Electronica AFIP), 2026-09-06
// (retomado -- ver docs/DECISIONS.md). Logica identica al clasico, salvo el dialogo de
// #btnGenerarNotaCredito: WebCore.VentasController.GenerarNotaCredito NO soporta anularVenta
// (recorte deliberado y ya documentado en el controller, ver su propia cabecera) -- se ofrece
// directo "Generar nota de credito" en vez del choice "Generar y anular venta" / "Generar sin
// anular" del clasico, para no ofrecer una opcion que el servidor ignoraria en silencio.
(function () {
    ...
    // rechazaba el POST con 400 y el jQuery .fail() mostraba "Error en la peticion", dejando la
    // venta colgada (bug real reportado probando "Cerrar sin facturar", 2026-08-21, ver
    // docs/DECISIONS.md). Mismo fix en los 5 $.post de este archivo: leer el token a mano y
    // mandarlo siempre por header. (WebCore.VentasController no exige antiforgery en estas
    // acciones hoy -- se mantiene igual por fidelidad y por si se agrega mas adelante.)
    ...
    $(document).on('keydown', function (e) {
        ...
        // Supr: cerrar venta sin facturar. Sin modificador, asi que a diferencia de un atajo con
        // Alt hay que ignorar el foco en campos de texto -- si no, borrar texto seleccionado con
        // Supr en Razon Social, Observaciones, etc. dispararia el cierre en vez de borrar.
    ...
    function seleccionarPersonaFactura($fila) {
        ...
        // El formato del domicilio replica el que arma BuildFacturaDTO (VentasController.cs) para
        // una venta real: "Domicilio - Ciudad".
        if (iva) {
    ...
    // Elegir "Responsable Inscripto" fuerza Tipo de Factura = A. Corre tanto al elegir cliente
    // (seleccionarPersonaFactura dispara "change" a mano) como si el usuario cambia el select de
    // Cond. IVA directamente -- no interfiere con el calculo server-side
    // (FacturaElectronica.getCodTipoCbteAFIP), que solo corre una vez al abrir el modal; esto es
    // puramente reactividad dentro del modal ya abierto.
    ...
    $(document).on('click', '#btnGenerarNotaCredito', function () {
        ...
        Swal.fire({
            ...
            text: '¿Confirmás generar la nota de crédito para esta factura?',
            showCancelButton: true,
            confirmButtonText: 'Generar nota de crédito',
            cancelButtonText: 'Cancelar'
        }).then(function (result) {
