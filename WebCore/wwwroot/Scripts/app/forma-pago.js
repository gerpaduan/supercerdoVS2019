// Port literal de Web/Scripts/app/forma-pago.js (batch 3, ver
// docs/10-migracion-aspnet-core/PLAN-POS-UI.md). Sin cambios de logica -- finalizarVenta() postea
// a window.api.venta.finalizar/modificar (definidos en Ventas/POS.cshtml), ya portados y
// verificados contra la base real (ver PLAN-POS.md). window.mostrarModalPostVenta esta
// sobreescrito en POS.cshtml con una version simple (Swal + reload) porque modal-postventa.js
// (1298 lineas, ticket/email/AFIP) todavia no esta portado -- llega en el batch 6.
// ===============================
// VARIABLES GLOBALES
// ===============================
...
// CAMBIO DELIBERADO respecto al original (2026-09-04, autorizado explicitamente por el usuario --
// ver docs/DECISIONS.md): en el Web clasico, pago mixto solo se habilitaba si el negocio tiene
// preseleccion de forma de pago activa (precios distintos segun forma de pago, ver
// ObtenerConfiguracionFormaPagoPOS en VentasController.cs) Y esa preseleccion coincide con la
// forma de pago elegida como "otro medio". El usuario pidio desacoplar esto: "Pago Mixto" se
// puede tildar siempre en modo finalizacion, sin necesitar preseleccion -- la unica regla es que
// el "otro medio" (la forma de pago que se mezcla con Efectivo) no puede ser Efectivo ni CtaCte.
function bloquearFormasPagoNoPreseleccionadas() {
    ...
    // Sin preseleccion real: el unico bloqueo es el propio de pago mixto (no puede combinarse
    // con Efectivo -- ya está cubierto por Efectivo -- ni con CtaCte, que no admite pago mixto).
    const esPagoMixto = $('#chkPagoMixto').is(':checked');
    $('.btn-forma-pago').each(function () {
        const tipo = normalizarTipoFormaPago($(this).data('tipo'));
        const excluidoPorMixto = esPagoMixto && (tipo === 'Efectivo' || tipo === 'CtaCte');
        $(this).prop('disabled', ventaEnProceso || excluidoPorMixto);
    });
}
...
function configurarModalFormaPagoSegunModo() {
    const modo = getModoFormaPagoActual();
    // Pago mixto disponible en modo finalizacion siempre -- ya no depende de haber
    // preseleccionado una forma de pago (ver el comentario de bloquearFormasPagoNoPreseleccionadas
    // mas arriba y docs/DECISIONS.md, cambio deliberado 2026-09-04).
    const mostrarPagoMixto = modo === 'finalizacion';
