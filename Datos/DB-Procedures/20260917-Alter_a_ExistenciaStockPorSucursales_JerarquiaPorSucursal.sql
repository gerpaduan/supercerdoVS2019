...
-- ============================================================================
-- Feature: excepcion de independiente/enCierreStock por sucursal (ver
-- docs/DECISIONS.md, 2026-09-17, y Datos/DB-Procedures/20260917-Create_CorteJerarquiaSucursal.sql).
--
-- Problema real: Corte.independiente/Corte.enCierreStock son columnas por (idEmpresa,
-- idCorte), sin dimension de sucursal. Un corte como "Malvina" puede depender de "Carre"
-- en una sucursal (San Martin) pero ser independiente en otra (San Lorenzo) -- misma
-- empresa, dos sucursales. Hoy la jerarquia recursiva (SelfMap/Descendientes/Ascendientes/
-- Mapa/MapaCorte) se calcula UNA sola vez para todas las sucursales pedidas en la corrida,
-- sin distincion.
--
-- Fix: se arma #CorteEfectivoPorSucursal (grano idCorte+idSucursal) ANTES de la CTE
-- recursiva, resolviendo el valor EFECTIVO (override de dbo.CorteJerarquiaSucursal si
-- existe, si no el valor global de dbo.Corte) de independiente/enCierreStock para cada
-- combinacion. El idCorteMaestro NUNCA se overridea (sigue siendo siempre el global) --
-- el override solo puede marcar "independiente"/"en cierre de stock" en una sucursal
-- puntual, nunca redirigir a un corte maestro distinto.
--
-- #MapaCorte gana la columna idSucursal en su clave. SelfMap/Descendientes/Ascendientes/
-- Mapa pasan a resolver independiente vs #CorteEfectivoPorSucursal en vez de dbo.Corte
-- directo. Los joins de Descendientes/Ascendientes ganan una condicion extra
-- (ceHijo.independiente = 0 / ceActual.independiente = 0): un corte marcado independiente
-- por excepcion en una sucursal queda excluido de la propagacion hacia/desde su corte
-- maestro EN ESA SUCURSAL PUNTUAL -- sin esto, esa sucursal contaria el stock dos veces
-- (una vez como propio via SelfMap, otra sumado/derivado del padre via
-- Descendientes/Ascendientes). Verificado que esto aplica simetricamente en las dos
-- direcciones de la jerarquia (Descendientes: comprar el PADRE acredita un rendimiento
-- implicito al HIJO via porcentaje; Ascendientes: comprar el HIJO sube al stock del PADRE
-- cuando el hijo no es independiente) -- ambas quedan excluidas para el hijo marcado
-- independiente por excepcion en esa sucursal.
--
-- Los 8 INSERT a #Operaciones (este SP no tiene StockCierre, a diferencia de
-- a_CierreStockWeb) ganan "AND mc.idSucursal = X.idSucursal" en su join a #MapaCorte.
--
-- #AllCortes cambia su filtro de "c.independiente = 1 AND ISNULL(c.enCierreStock,0) = 1"
-- (global) a "ce.independiente = 1 AND ce.enCierreStock = 1" (efectivo, por sucursal, via
-- #CorteEfectivoPorSucursal) -- esto es lo que hace que un corte con excepcion aparezca
-- como fila propia SOLO en la sucursal donde se marco.
--
-- Aplicado primero contra la base local supercerdo (SQL Server, .\sqlexpress) para probar
-- con datos reales antes de replicar a Postgres (ver docs/DECISIONS.md). NO se aplica a
-- los servidores reales de San Martin/San Lorenzo en este cambio.
-- ============================================================================
...
BEGIN
    ...
    -- Resuelve, para cada Corte x Sucursal candidata, el valor EFECTIVO de
    -- independiente/enCierreStock: el override de dbo.CorteJerarquiaSucursal si existe
    -- para esa sucursal puntual, si no el valor global de dbo.Corte.
    --
    -- tieneExcepcionIndependiente (bug real encontrado y corregido en el desarrollo,
    -- 2026-09-17): NO es lo mismo que "independiente=1". Un corte puede ser independiente
    -- GLOBALMENTE (ej. "Carre") Y AL MISMO TIEMPO depender de otro corte independiente mas
    -- arriba en la cadena (ej. Carre.idCorteMaestro = "Media Res", que tambien es
    -- independiente) -- comprar "Media Res" reparte rendimiento tanto a su propio stock
    -- como al de Carre, simultaneamente, por diseño (fan-out, no un unico camino). Si la
    -- exclusion de Descendientes/Ascendientes usara "independiente=0" a secas, cortaria
    -- por error esta cadena para CUALQUIER corte que ya sea independiente por default
    -- (rompe el calculo de Carre en las dos sucursales, confirmado con el snapshot de
    -- antes/despues). La exclusion real que hace falta es mas especifica: cortar la
    -- cadena SOLO cuando hay una EXCEPCION activa marcando independiente=1 para esa
    -- sucursal puntual -- nunca por el valor global, sea cual sea.
    ...
    (
        SELECT
            CAST(f.Compras + f.IngresoElaborado + f.IngresoStock + f.IngresoMovimiento + f.AjusteStock AS decimal(18, 3)) AS TotalIngresos,
            CAST(f.EgresoStock + f.EgresoMovimiento + f.EgresoElaborado + f.Ventas AS decimal(18, 3)) AS TotalEgresos,
            CAST(
