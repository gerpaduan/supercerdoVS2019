...
BEGIN
    ...
    WHERE
        ...
        -- Filtro de negocio propio de esta consulta (no alcanza con la RLS "empresa o
        -- catalogo global"): esta pantalla no debe traer el catalogo global (idEmpresa=0),
        -- solo los productos propios de la empresa. Mismo patron ya aplicado el 2026-08-01
        -- directo en la base local (nunca se habia versionado el script hasta ahora) y
        -- documentado en docs/07-operacion-y-soporte/incidencias-frecuentes.md.
        AND (@idEmpresa IS NULL OR c.idEmpresa = @idEmpresa)
    ...
    -- (Compras/CortePorCompra), separados por tipoCompra. cantKg de "Egreso
    -- Stock" ya viene NEGATIVO desde StockController.cs (guarda cantidad*-1),
    -- asi que se multiplica por -1 aca para que EgresoStock quede como
    -- magnitud positiva -- igual que EgresoMovimiento/EgresoElaborado/Ventas
    -- (fix Hallazgo 1). Las otras 3 categorias no se tocan, se guardan
    -- siempre positivas.
    INSERT INTO #Operaciones
    (
        TipoOperacion,
